  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__appzaThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__appzaArtifact__', '__appzaThought__', '__appzaQuickAction', '__appzaSelectedElement__'],

      // ['className', '__appzaThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__appzaArtifact__', '__appzaThought__', '__appzaQuickAction'],
    ],
  },
  strip: [],
};
