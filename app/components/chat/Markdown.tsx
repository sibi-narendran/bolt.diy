    ...
    const components = useMemo(() => {
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes('__appzaArtifact__')) {
          ...
          if (className?.includes('__appzaSelectedElement__')) {
            const messageId = node?.properties.dataMessageId as string;
          ...
          if (className?.includes('__appzaThought__')) {
            return <ThoughtBox title="Thought process">{children}</ThoughtBox>;
          }

          if (className?.includes('__appzaQuickAction__') || dataProps?.dataappzaQuickAction) {
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            dataProps?.class?.toString().includes('__appzaQuickAction__') ||
            dataProps?.dataappzaQuickAction === 'true'
          ) {
 ...
 * const input = "```xml\n<div class='__appzaArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__appzaArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact (marked with __appzaArtifact__ class)
 * - Handles code fences with optional language specifications (e.g. ```xml, ```typescript)
 * - Preserves original content if no artifact is found
 * - Safely handles edge cases like empty input or artifacts at start/end of content
 */
export const stripCodeFenceFromArtifact = (content: string) => {
  if (!content || !content.includes('__appzaArtifact__')) {
    return content;
  }

  const lines = content.split('\n');
  const artifactLineIndex = lines.findIndex((line) => line.includes('__appzaArtifact__'));
