...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          appza_user_profile: this._safeGetItem('appza_user_profile'),
          appza_settings: this._safeGetItem('appza_settings'),
          appza_profile: this._safeGetItem('appza_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('appza_viewed_features'),
          developer_mode: this._safeGetItem('appza_developer_mode'),
        ...
        ui: {
          // Tab configuration
          appza_tab_configuration: this._safeGetItem('appza_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('appza_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('appza_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          appza_read_logs: this._safeGetItem('appza_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('appza_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('appza_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.appza_tab_configuration) {
        try {
          this._safeSetItem('appza_tab_configuration', data.ui.appza_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'appza_acknowledged_debug_issues',
        'appza_acknowledged_connection_issue',
        'error_logs',
        'appza_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('appza_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
