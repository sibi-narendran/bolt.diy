...
export function simplifyappzaActions(input: string): string {
  // Using regex to match appzaAction tags that have type="file"
  const regex = /(<appzaAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/appzaAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<appzaAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</appzaAction>`;
    });

  return `<appzaArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</appzaArtifact>`;
}
