...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyappzaActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyappzaActions(content);

      content = content.replace(/<div class=\\"__appzaThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`appzap failed to select files`);
  }
